...
trait HostParser extends RichStringBuilding {
  ...
  def `dec-octet`: Rule0 = rule {
    capture((1 to 3).times(Digit)) ~> { strByte: String =>
      val byte = strByte.toInt
      test(byte < 256) ~ appendSB(byte.toString)
    }
  }

  def IPv4address: Rule1[String] = rule {
    clearSB() ~
      `dec-octet` ~ '.' ~ appendSB() ~ `dec-octet` ~ '.' ~ appendSB() ~ `dec-octet` ~ '.' ~ appendSB() ~ `dec-octet` ~
      push(sb.toString)
  }
...
//
  def `ireg-name`: Rule1[String] = rule {
    clearSB() ~
      (iunreserved | `pct-encoded` | `sub-delims`).* ~
      push(sb.toString.toLowerCase)
  }
